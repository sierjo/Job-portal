...
import com.spring_boot_project.jobportal.entity.IRecruiterJobs;
...
public interface JobPostActivityRepository extends JpaRepository<JobPostActivity, Integer> {
    ...
    * Retrieves list of jobs for a given recruiter id
            Combines/joins
    job post activity, job location,
               company
    Also includes a count of job seekers/candidates
    *   who have applied for a given job
    * */
    ...
    List<JobPostActivity> search(@Param("job") String job,
                                 @Param("location") String location,
                                 @Param("remote") List<String> remote, List<String> type,
                                 @Param("type") LocalDate searchDate);
